...
export const register = async (req, res) => {
    ...
    try {
        ...
        // Store email in cookie for verification
        res.cookie('userEmail', email, { maxAge: 86400000 });

        // Inline email content
        const html = `
            <div style="font-family: Arial, sans-serif; padding: 20px;">
                <h2>Email Verification - Prescripto</h2>
                <p>Hello ${name},</p>
        ...
        return res.status(201).json({ success: true, message: 'User registered. OTP sent to email.' });
...
export const verifyOTPs = async (req, res) => {
    const { otp } = req.body;
    const userEmail = req.cookies.userEmail;  // Access email from cookies
    ...
    if (!userEmail) {
        return res.status(400).json({ success: false, message: 'Email not found in cookies' });
    }
